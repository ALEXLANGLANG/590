...
class STE(torch.autograd.Function):
    ...
    def forward(ctx, w, bit):  # "w" is the weight tensor (as a PyTorch Tensor), "bit" is the precision to be quantized to. You can ignore "ctx" as it will not be used
        if bit is None:
            ...
            # Your code here:
            alpha =        # Compute alpha (scale) for dynamic scaling
            beta =         # Compute beta (bias) for dynamic scaling
            ws =           # Scale w with alpha and beta so that all elements in ws are between 0 and 1
            
            R =            # Quantize ws with a linear quantizer to "bit" bits
            
            wq =           # Scale the quantized weight R back with alpha and beta
